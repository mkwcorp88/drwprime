...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { updated, isOrderReadyForConfirmation } = await completeAction(performer, id, typeof body.note === 'string' ? body.note.trim() : undefined);

    return NextResponse.json({
      action: updated,
      isOrderReadyForConfirmation,
    });
  } catch (error) {
