...
import { getMonthRange, getPeriodRange, jakartaPeriod, OpsError, parseOpsMonth, type PeriodKey } from '@/lib/treatment-operations/utils';
...
export async function GET(request: Request) {
  try {
    ...
    const month = period === 'month' ? parseOpsMonth(url.searchParams.get('month') ?? jakartaPeriod(new Date())) : '';
    const range = period === 'month'
      ? getMonthRange(month)
      : getPeriodRange(period, url.searchParams.get('start') ?? undefined, url.searchParams.get('end') ?? undefined);
