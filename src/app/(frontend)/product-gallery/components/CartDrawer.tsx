...
function InputField({
  ...
  const baseClass = "w-full border rounded-lg px-4 py-2.5 text-sm transition-colors bg-stone-50 text-stone-800 placeholder-stone-400 focus:outline-none ";
  const borderClass = error ? 'border-red-300 focus:border-red-500' : 'border-stone-200 focus:border-stone-400';

  return (
    <div>
      <label htmlFor={id.current} className="block text-[11px] font-medium mb-1 text-stone-500">{label}</label>
      {multiline ? (
      ...
      <div className="absolute inset-0 lg:right-auto lg:w-full lg:max-w-md" onClick={e => e.stopPropagation()}>
        <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Keranjang" tabIndex={-1} className="h-full flex flex-col bg-white shadow-xl outline-none lg:border-r lg:border-stone-100">
          ...
          <div className="flex-1 overflow-y-auto">
            {displayOrder ? (
              <div className="p-5 text-center">
                ...
                <h3 className="font-bold text-xl mb-2 text-stone-800">Pesanan Dibuat!</h3>
                <p className="text-stone-500 text-sm mb-6">Kamu akan diarahkan ke halaman pembayaran.</p>
                {displayOrder.paymentUrl && (
                  <a
                    href={displayOrder.paymentUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-semibold text-sm text-white transition-all hover:shadow-lg"
                ...
                {displayOrder.publicToken && (
                  <a
                    ...
                    className="block mt-4 text-amber-600 text-sm hover:underline"
                  >
              ...
              <div className="text-center py-16">
                <svg className="w-16 h-16 mx-auto mb-4 text-stone-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" />
                </svg>
                <p className="text-stone-400 text-sm">Keranjang masih kosong</p>
              </div>
            ) : (
              <div className="p-5 space-y-4">
                {cart.map(item => (
                  <div key={item.product.id} className="flex gap-3 py-3 border-b border-stone-100 last:border-0">
                    <div className="w-16 h-20 rounded-xl flex items-center justify-center shrink-0 bg-stone-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm line-clamp-2 text-stone-800">{item.product.name}</h4>
                      <p className="text-[10px] mt-0.5 text-stone-400">{item.product.size}</p>
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 pt-4 border-t border-stone-100" noValidate>
                  <h3 className="font-bold text-sm text-stone-700">Data Penerima</h3>
          ...
          {cart.length > 0 && !displayOrder && (
            <footer className="border-t border-stone-100 p-5 space-y-3 flex-shrink-0 bg-white">
              <div className="flex items-center justify-between">
                <span className="text-sm text-stone-500">Total</span>
                <span className="font-bold text-xl text-stone-800">{formatPrice(total)}</span>
              </div>
              {apiError && <p className="text-red-500 text-xs text-center">{apiError}</p>}
              <button
                onClick={() => formRef.current?.requestSubmit()}
                disabled={loading}
                className="w-full py-3.5 rounded-xl font-semibold text-base transition-all disabled:opacity-40 flex items-center justify-center gap-2"
